...
// Modified version of EmuList for PT-ModManager
function EmuListNew(x, y, width, element_height, content_slots, callback) : EmuCallback(x, y, width, 50, "", 0, callback) constructor {
    ...
    self.Render = function(base_x, base_y, debug_render = false) {
        ...
        if (n == 0) {
            ...
            for (var i = 0; i < min(n, self.slots); i++) {
                ...
                var ya = y2 + self.element_height * i;
                var yb = ya + self.element_height;
                var tya = mean(ya, yb);
        ...
        if (self.getMouseHover(lx1, ly1, lx2, ly2)) {
            var mn = min(((self.getMousePositionY() - ly1) div self.element_height) + self.index, n - 1);
